...
class tempGraph:
	...
	def closeEnought(self,other,tol):
		...
		# Allows the values to be in another order, as long as it is a rearrangement of the rows
		t5 = checkIfPerm(self.obsVal, other.obsVal)

		# ObsIdx only contains indexes, if these indexes are rearranged they change but 
		#it must be maintained that Ga.vals [gA.ObsIdx] = GB.vals [gB.ObsIdx] 
		#They can still come out of order
		# TODO INEFFICIENT AS THEY INCREASE VALUES
...
def createGraph(id1,id2,focal,pA,pB,Rt,f):
	...
	graph.focal = focal
	graph.f = f
	graph.mot = np.zeros((3,4,2)) # 3X (4 row X2 colum)
	n = pA.shape[0]

	graph.mot[:,:,0] = np.hstack([np.eye(3),np.zeros((3,1))]) #eye(3):3X3的单位矩阵 3 x 4
	graph.mot[:,:,1] = Rt
...
def triangulateGraph(graph,imagesize):
	...
	for i in range(n):
		...
		cnt=0
		#Get the points in the plane of the camera and the projection matrix
		for ind in validCamera:
			x[cnt,:] = newGraph.obsVal[int(newGraph.ObsIdx[i][ind]),:]
